...
| 위협            | 경로                                                                 | 방어                                                                                                                                      |
| --------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| 세션 탈취       | XSS로 토큰 읽기, localStorage 덤프, 로그에 남은 토큰                 | 토큰은 `httpOnly` 쿠키에만 — JS가 못 읽는다. 로그·URL에 토큰 금지                                                                         |
...
| 비밀 유출       | `NEXT_PUBLIC_`에 secret, `.env` 커밋, 소스맵 공개                    | 서버 전용 env, `.env*` gitignore, 브라우저 소스맵 off                                                                                     |
...
| 헤더                        | 값                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Content-Security-Policy`   | `default-src 'self'; script-src 'self' 'nonce-…' https://accounts.google.com https://t1.kakaocdn.net; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https: <CONTENT_IMAGE_ORIGINS>; font-src 'self'; connect-src 'self' https://accounts.google.com https://kauth.kakao.com; frame-src https://accounts.google.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'; upgrade-insecure-requests` — 도메인 목록은 구현 시 실제 SDK 요구에 맞춰 확정하고 여기 갱신한다 |
...
- 외부 스크립트는 카카오·구글 로그인 SDK(로그인 페이지)뿐. 새로 추가하면 위 표와 `next.config.ts`를 함께 고친다.
...
- `img-src`는 `https:` 전체를 연다(LAN-440). 답장·편지 본문 미리보기가 앱과 같은 그림이어야 하는데 앱은 CSP 없이 외부 이미지를 그리므로, 어드민만 막으면 "앱에선 보이는데 미리보기엔 안 보이는" 어긋남이 생긴다. 열어서 생기는 위험은 어드민 화면이 임의의 외부 서버에 이미지 요청을 보내 관리자 IP가 남는 정도이고, `Referrer-Policy`가 URL을 숨긴다. 업로드 대상(`connect-src`)은 여전히 스토리지 오리진뿐이다.
...
이 저장소는 공개 예정이다. **설계는 공개를 견딘다** — 쿠키 이름·CSP·CSRF 판정 방식이 알려져도 약해지지 않는 것들이고(비밀에 기대는 방어가 없다), 자격증명은 코드에 없다(전부 Vercel 환경변수). landit-fe·landit-be가 이미 공개라 API 형태도 새 정보가 아니다.

바뀌는 건 하나다 — **어드민 주소와 "무엇을 어떻게 막고 있는지"가 함께 알려진다.** 정찰 비용이 내려가므로, 문 자체를 한 겹 더 잠그고 넘어간다.

전환 전 확인.

- [ ] **히스토리에 비밀이 없다** — `.env*`는 `.env.example`(값 전부 빈칸)만 커밋됨, 토큰·키 패턴 없음. 확인 완료(2026-08-20)
- [ ] **Vercel Deployment Protection을 Production까지 켠다** — private일 땐 "권장"이었지만, 주소가 공개되면 로그인 화면 앞에 문이 하나 더 있어야 한다. 팀원이 Vercel 계정을 갖고 있어야 하므로 플랜·인원 확인 필요
- [ ] **`@claude` 워크플로가 쓰기 권한자로 제한돼 있다** — `author_association` 검사. 확인 완료
- [ ] **Vercel Firewall** — `/api/auth/*` rate limit. 공개 후에는 자동 시도가 늘 수 있다
- [ ] 카카오·구글 콘솔의 redirect URI가 `admin.landit.im` 경로만 등록돼 있는지 (임의 URI로 코드 교환을 유도할 수 없게)

**문서는 그대로 둔다.** 아래 "후속" 목록처럼 "아직 안 한 것"을 적어두는 게 공격자에게 안내판이 되지 않느냐는 우려가 있는데, 적힌 항목들은 코드를 읽으면 그대로 드러나는 구조적 사실이고(관리자 판정을 403으로 한다는 것 등), 숨긴다고 막히지 않는다. 숨겨서 얻는 이득보다 다음 사람이 배경을 아는 이득이 크다.
