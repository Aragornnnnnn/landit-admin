// 브라우저 소셜 로그인 진입 — 카카오 JS SDK authorize, 구글 PKCE redirect (landit-fe 웹 폴백 그대로, next 보존만 추가)
import {
  createCodeChallenge,
...
export interface PendingSocialLogin {
  ...
  next?: string; // 로그인 후 돌아갈 경로 (같은 오리진 상대 경로만 — safeNextPath로 검증한 값)
}

const SOCIAL_LOGIN_STORAGE_KEY = 'landit-admin-social-login';
